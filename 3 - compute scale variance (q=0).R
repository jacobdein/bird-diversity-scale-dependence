...
df <- hbins_estD %>%
  ...
  select(id, qD_0) %>%
  rename(id_level1 = id, qD_0_level1 = qD_0)
...
join_parent_bins <- function(leveln) {
  return(
    st_join(
      df,
      filter(hbins_estD, level == leveln) %>% select(id, qD_0),
      join = st_within,
      left = TRUE,
...
df <- df %>%
  filter(!is.na(qD_0_level1))

# TEMP: remove all 'value' columns for all levels except level1
df <- df %>% st_drop_geometry() %>% rename(X_level1 = qD_0_level1) %>% select(-starts_with("qD")) %>% rename(qD_0_level1 = X_level1)

# add column for final level (global value)
df <- df %>%
  mutate(id_level6 = max(hbins_estD$id)+1,
         qD_0_level6 = mean(qD_0_level1))
...
compute_level_mean <- function(leveln) {
  ...
  group_by(!!sym(glue("id_level{leveln}"))) %>%
    summarise("qD_0_level{leveln}" := mean(!!sym(glue("qD_0_level{leveln-1}")))) %>%
    left_join(df, .)
}
...
compute_squares <- function(leveln) {
  return(
    df %>%
      transmute("sq_level{leveln}" := (!!sym(glue("qD_0_level{leveln+1}")) - !!sym(glue("qD_0_level{leveln}")))^2)
  )
}
...
compute_sv <- function(leveln) {
  return(
    df %>% 
      group_by(!!sym(glue("id_level{leveln}"))) %>%
      summarise(col1 = mean(!!sym(glue("qD_0_level{leveln}"))), col2 = mean(!!sym(glue("qD_0_level{leveln+1}")))) %>%
...
tss <- sum((df$qD_0_level1 - df$qD_0_level6)^2)
...
sv_p <- sv %>% 
  ...
  write_csv("../results/R1 - London 1000m 5L/sv_q0.csv")
...
ggplot(aes(x=scale, y=sv_cumulative, label = scale)) +
  ...
  ggtitle('Scale variance of bird diversity (Hill number q = 0)')

ggsave("../results/R1 - London 1000m 5L/scale_variance_q0.png")


# --------- plot for all q --------------- #
# load results
sv_p <- read_csv("../results/R1 - London 1000m 5L/sv_p.csv") %>%
  mutate(q = as_factor(q))

# plot results
sv_p %>%
  ggplot(aes(x=scale, y=sv_cumulative, label = ifelse(q==2, scale, ""), group = q, color = q)) +
  scale_x_continuous(trans='log10') +
  #geom_col(width = 0.001, fill='black') +
  geom_point(size=3) + 
  ...
  xlab('scale (m)') +
  ylab('Cumulative variance (%)') + 
  ggtitle('Scale variance of bird diversity (Hill number q = (0, 1, 2))')

ggsave("../results/R1 - London 1000m 5L/scale_variance.png")
